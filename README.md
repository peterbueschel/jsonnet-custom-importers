...
## Custom Importers
  ...
  - The pattern for the import path is `import` `'<importer prefixa>` `<separator>` `<glob pattern or filepath or url>'` . (the default `<separator>` is `://`).
...
where `glob.stem+` is one of the possible `<importer prefixa>`, the `://` is the `<separator>` to the final `<glob pattern>` (here: `**/*.jsonnet`)
...
``` go
  m := NewMultiImporter(NewGlobImporter(), FallbackFileImporter())
```
